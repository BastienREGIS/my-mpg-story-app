...
export default async function AdminMatchResultsPage() {
    ...
    leagues.map(async (l) => {
      ...
      if (!season) {
        return {
          ...
          name: l.name,
          seasonId: null,
          teams: [] as { id: string; label: string }[],
        }
      }
  ...
  const formEnabled = Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY && process.env.ADMIN_MATCH_ENTRY_SECRET)
          ...
          <>
            <section className="space-y-3 rounded-xl border border-border bg-card/50 p-4 sm:p-5">
              ...
              <ol className="list-decimal space-y-2 pl-5 text-sm text-muted-foreground">
                <li>Noter les scores de la journée.</li>
                <li>
                  Les insérer dans <strong className="text-foreground">matches</strong> (éditeur Supabase — détail
                  dans <code className="rounded bg-muted px-1">docs/WEEKLY_WORKFLOW.md</code>).
                </li>
                <li>
                  Ouvrir le{" "}
                  <Link href="/" className="text-primary underline-offset-4 hover:underline">
